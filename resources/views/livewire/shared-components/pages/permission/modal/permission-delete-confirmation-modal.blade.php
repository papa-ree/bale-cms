...
new class extends Component {
    ...
    public function deletePermission(LivewireAlert $alert, $permission_id)
    {
        ...
        try {
            ...
            $this->dispatch('closeBaleModal', id: 'permissionDeleteModal');
            $this->dispatch('refresh-permission-list');

            $alert->title('Permission Deleted!')->position('top-end')->success()->toast()->show();
        } catch (\Throwable $th) {
    ...
    {
        return Permission::find($this->permission_id)->delete();
    }
}; ?>
...
<div x-data="{
    ...
    <div class="sm:flex sm:items-start">
        ...
        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
            <h3 class="text-base font-semibold leading-6 text-gray-900 dark:text-white" id="modal-title">
                Delete <span x-text="permissionName"></span> permission?
            </h3>
            <div class="mt-2">
    ...
    <x-bale.modal-action>
        <x-bale.secondary-button label="Cancel"
            wire:click="$dispatch('closeBaleModal', { id: 'permissionDeleteModal' })" class="ml-3" />
        <x-bale.danger-button label="Gaskeun!" @click="$wire.deletePermission(permissionId); useSpinner()" />
    </x-bale.modal-action>
